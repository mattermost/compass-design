...
- Brand SVG illustrations live in `src/illustrations/*.svg` and publish as `@mattermost/compass-ui/illustrations/<kebab-name>`. After adding or renaming an SVG, run `npm run generate:illustrations` from the repo root.
