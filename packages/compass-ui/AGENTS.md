...
Brand SVGs ship from `@mattermost/compass-ui/illustrations/<name>` (tree-shakeable subpaths). Production code imports a named SVG and passes it as `glyph` or `children` on `Illustration` — there is no string `name` prop on the published component.

**Storybook stories** for illustration slots: use `illustrationSelectArgType` + `resolveStoryIllustration` from `src/storybook/illustrations.tsx` (registry generated by `npm run generate:illustrations`). Prefer a meta-level `render` that maps select values (`None` / illustration name) to React nodes so Controls can swap artwork. Do not put React elements in `args` for those props when a select control is present.
...
- **Illustration controls:** for props that take brand SVG artwork (`glyph`, `illustration`, …), use `illustrationSelectArgType` + `resolveStoryIllustration` from `src/storybook/illustrations.tsx` (registry generated by `npm run generate:illustrations`). Same Controls pattern as icons — string select values, not React elements in `args`.
